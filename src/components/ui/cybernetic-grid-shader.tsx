...
const CyberneticGridShader = () => {
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // base grid color pulsing (Fluentia Neon Green: #00F38D -> vec3(0.0, 0.953, 0.553))
        // using the muted secondary green 
        vec3 gridColor = vec3(0.0, 0.953, 0.553) * 0.4;
        vec3 color     = gridColor
                       * line
                       * (0.5 + sin(t * 2.0) * 0.2);

        // energetic pulses along grid (Fluentia Neon Green)
        float energy = sin(uv.x * 20.0 + t * 5.0)
                     * sin(uv.y * 20.0 + t * 3.0);
        energy = smoothstep(0.8, 1.0, energy);
        color += vec3(0.0, 0.953, 0.553) * energy * line;

        // glow around mouse (Subtle green glow)
        float glow = smoothstep(0.1, 0.0, mouseDist);
        color += vec3(0.0, 0.953, 0.553) * glow * 0.15;
  ...
  return (
    <div
      ref={containerRef}
      className="shader-container absolute inset-0 w-full h-full z-0 overflow-hidden"
